...
from data import get_dataloaders, get_dataloaders_proxy, get_testaug_dataloaders_proxy
...
def run_epoch(model, loader, loss_fn, optimizer, desc_default='', epoch=0, writer=None, verbose=1, scheduler=None):
    ...
    return metrics
def run_epoch_search(model, loaders, val_loader, loss_fn, optimizer,optimizer_aug, optimizer_tp, aug_param,tp_param, desc_default='',explore_ratio = 0, w0s_at=[],w0s_mt=[], ops_num = 2, epoch=0, writer=None, verbose=1, scheduler=None,dict_reward={}):
    tqdm_disable = bool(os.environ.get('TASK_NAME', ''))  
    transform_or = copy.deepcopy(loaders[0].dataset.transform)
    transform_or.transforms.pop(0)
    print(transform_or)
    #do resize and crop first!
    loaders[0].dataset.transform = transforms.Compose([
    ...
    for data in loader_t:
        ...
        for idl in range(1,len(loaders)):
            ...
            trace_loss = -1/gd_norm * gip_ls[idl].data.item() * torch.sigmoid(tp_param) * trace_prob(aug_param, idxs)/Z
            trace_loss.backward()
...
def train_and_eval(tag, dataroot, test_ratio=0.0, cv_fold=0, reporter=None, metric='last', save_path=None, only_eval=False, reduct_factor=1.0, args = None):
    ...
    optimizer_aug = torch.optim.Adam((aug_param,),lr=args.param_lr, betas=(0.5, 0.999))
    optimizer_tp = torch.optim.Adam((tp_param,),lr=args.tp_lr, betas=(0.5, 0.999))
    trainsampler, _, _, testloader_ = get_dataloaders_proxy(C.get()['dataset'], C.get()['batch'], dataroot, test_ratio, split_idx=cv_fold)
    ...
    for epoch in range(epoch_start, max_epoch + 1):
        ...
        loaders = [get_testaug_dataloaders_proxy(
            C.get()['dataset'], C.get()['batch'], dataroot, train_sampler = trainsampler)]

        for i in range(args.loader_num - 1):
            AugType = (args.ops_num, select_op(aug_param, args.ops_num))
            loaders.append(get_testaug_dataloaders_proxy(
                C.get()['dataset'], C.get()['batch'], dataroot, 
                train_sampler = trainsampler, AugType = AugType))
        for loader in loaders[1:]:
            print((loader.dataset.transform.transforms[2].n,loader.dataset.transform.transforms[2].idxs))
        
        rs = dict()
        #test loader is based on the val set actually!
        rs['train'] = run_epoch_search(
            model, loaders, testloader_, criterion, optimizer,optimizer_aug, optimizer_tp, aug_param, tp_param,
            explore_ratio = args.explore_ratio, ops_num = args.ops_num, desc_default='train', epoch=epoch, 
            writer=writers[0], verbose=True, scheduler=scheduler, dict_reward=dict_reward, w0s_at=w0s_at, w0s_mt = w0s_mt)
        #rs['train'] = run_epoch(model, trainloader_new, criterion, optimizer, desc_default='train', epoch=epoch, writer=writers[0], verbose=True, scheduler=scheduler)
        model.eval()
...
if __name__ == '__main__':
    ...
    #May 6th add the aug_reduct_factor to control the porb decreasing for all aug ops!
    parser.add_argument('--rf', type=float, default=2.0)
    parser.add_argument('--cv-ratio', type=float, default=0.0)
    parser.add_argument('--mul', type=float, default=1.5)
    parser.add_argument('--cv', type=int, default=0)
    parser.add_argument('--load_tp', type=str, default='none')
    parser.add_argument('--only-eval', action='store_true')
    parser.add_argument('--loader_num', type=int, default=8)
