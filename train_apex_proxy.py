...
from data import get_dataloaders_proxy
...
def train_and_eval(tag, dataroot, test_ratio=0.0, cv_fold=0, reporter=None, metric='last', save_path=None, only_eval=False, reduct_factor=1.0, args = None):
    ...
    trainsampler, trainloader, validloader, testloader_ = get_dataloaders_proxy(C.get()['dataset'], C.get()['batch'], dataroot, test_ratio, split_idx=cv_fold)
    ...
    # train loop
    best_top1 = 0
    th_ls=[(1/reduct_factor)**ir for ir in range(100)]
    flag_load = 1
    # print(th_ls)
    for epoch in range(epoch_start, max_epoch + 1):
        if args.load_tp == 'none':
            th_epoch = 0.2 + 0.4 * (epoch - epoch_start) / (max_epoch + 1 - epoch_start)
        else:
            if flag_load == 1:
                tp = args.mul * np.load(args.load_tp)
                flag_load = 0
                print(tp.size)
                print(tp)
            if(epoch < tp.size):
